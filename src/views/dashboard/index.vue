<template>
  <div class="dashboard-container">
    ...
    <el-dialog :visible="showDialog" title="离职申请" @close="btnCancel">
      <!-- 表单内容 -->
      <el-form label-width="120px" ref="ruleForm" :model="ruleForm" :rules="rules">
        <el-form-item label="期望离职时间" prop="exceptTime">
          <!-- 离职时间 -->
          <el-date-picker v-model="ruleForm.exceptTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="请选择日期时间"></el-date-picker>
        </el-form-item>
        <el-form-item label="离职原因" prop="reason">
          <el-input v-model="ruleForm.reason" type="textarea" :autosize="{minRows: 3, maxRows: 8}" style="width: 70%;"></el-input>
        </el-form-item>
      </el-form>
      <!-- 确定取消 -->
    ...
    data() {
      return {
        ...
        ruleForm: {
          exceptTime: '',//离职时间
          reason: '',//离职理由
          processKey: 'process_dimission',//离职流程的标识
          processName: '离职'
        },
        rules: {
          exceptTime: [{ required: true, message: '离职时间不能为空', trigger: 'blur' }],
          reason: [{ required: true, message: '离职理由不能为空', trigger: 'blur' }]
        }
      }
    },
    methods: {
